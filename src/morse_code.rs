...
fn _decode_token(string: &str) -> String {
    _morse_to_alphanumeric_dictionary().get(string).unwrap_or(&_UNKNOWN_MORSE_CHARACTER).to_string()
}

fn _decode_part(string: &str) -> String {
    string.split(' ').map(_decode_token).collect::<Vec<String>>().join("")
}

pub fn decode(string: &str) -> Result<String, io::Error> {
    if !_check_all_parts(string) {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid morse code"));
    }
...
pub fn print_word_morse() {
    println!("Digite o quer criptografar");
    let mut message2 = String::new();
    stdin().read_line(&mut message2).expect("failed to readline");

    let cipher = encode(&message2);
    println!("{:?}", cipher);
    let data = decode(&cipher);
    println!("Descriptografado");
    println!("{:?}", data);
}
...
mod tests {
    ...
    fn decrypt_valid_morsecode_with_spaces() {
        let expected = "Hello Morse! How's it goin, \"eh\"?".to_string().to_uppercase();
